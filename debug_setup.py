...
def check_environment():
    ...
    if os.getenv("IN_GITPOD") == "true":
        if os.path.exists('.env.gitpod'):
            ...
            print_warning(".env.gitpod file not found - using system environment variables")
    else:
    ...
    if missing_keys:
        print_warning(f"Missing {len(missing_keys)} API keys. Some features may not work.")
        return False
    else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'results' in data and len(data['results']) > 0:
                ...
                print_success(f"Rebrickable API connection successful - Sample set: {set_name}")
                return True
            else:
                print_error("Rebrickable API returned no results")
                return False
        else:
            print_error(f"Rebrickable API error: {response.status_code} - {response.text}")
            return False
...
def test_brickowl_connection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'results' in data and len(data['results']) > 0:
                set_name = data['results'][0].get('name', 'Unknown')
                print_success(f"BrickOwl API connection successful - Sample set: {set_name}")
                return True
            else:
