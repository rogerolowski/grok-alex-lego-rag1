...
class SearchOptimizer:
    ...
    def test_search_parameters(self, query, k_values=[3, 5, 10], similarity_thresholds=[0.7, 0.8, 0.9]):
    ...
    def optimize_for_query_type(self, query):
        ...
        if any(word in query_lower for word in ["star wars", "marvel", "harry potter", "disney"]):
            # Theme-specific query
            return self._optimize_theme_search(query)
        elif any(word in query_lower for word in ["big", "large", "small", "pieces", "parts"]):
            # Size-specific query
            return self._optimize_size_search(query)
        elif any(word in query_lower for word in ["2024", "2023", "2022", "recent", "old"]):
            # Year-specific query
            return self._optimize_year_search(query)
        elif any(word in query_lower for word in ["expensive", "cheap", "price", "cost"]):
...
def main():
    ...
    if st.button("🔍 Test Search Parameters"):
        with st.spinner("Testing search parameters..."):
            ...
            st.success(f"Best parameters: {best_params[0]} (Score: {best_params[1]['avg_score']:.3f})")
    
    st.subheader("Optimized Search Strategies")
    
    strategy = st.selectbox("Choose search strategy:", [
        "General", "Theme-specific", "Size-specific", 
        "Year-specific", "Price-specific", "Hybrid", "Contextual"
    ])
