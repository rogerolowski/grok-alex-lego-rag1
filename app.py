...
def get_performance_metrics():
    ...
    try:
        ...
        # Cache status
        cache_dirs = ['/workspace/.cache/pip', '/workspace/.cache/uv', '.cache/pip', '.cache/uv']
        cache_status = {}
...
with st.sidebar:
    ...
    with st.expander("🔍 Advanced Filters"):
        # Theme filter
        themes = conn.execute("SELECT DISTINCT theme FROM lego_data WHERE theme IS NOT NULL").fetchall()
        theme_options = ["All Themes"] + [theme[0] for theme in themes]
        selected_theme = st.selectbox("Theme", theme_options)
        
        # Year range
        years = conn.execute("SELECT DISTINCT year FROM lego_data WHERE year IS NOT NULL ORDER BY year").fetchall()
...
if (query and search_button) or 'current_query' in st.session_state:
    ...
    with st.spinner("🔍 Searching LEGO database..."):
        try:
            ...
            tab1, tab2, tab3 = st.tabs(["🤖 AI Response", "📚 Source Documents", "📊 Analytics"])
            ...
            with tab2:
                ...
                if not filtered_docs:
                    st.warning("No results found with the current similarity threshold. Try lowering it.")
                else:
                    for i, (doc, score) in enumerate(filtered_docs, 1):
                        with st.expander(f"Record {i} (Score: {score:.3f})"):
                            try:
                                ...
                                with col1:
                                    st.markdown(f"**{data.get('name', 'Unknown Set')}**")
                                    st.markdown(f"**Set Number:** {data.get('set_number', 'N/A')}")
                ...
                if analytics_data:
                    ...
                    with col2:
                        ...
                        fig_year = px.histogram(
                            df, x='year', title="Results by Year",
                            nbins=10
                        )
