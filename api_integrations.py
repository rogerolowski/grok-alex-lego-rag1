        ...
        try:
            ...
            for endpoint in endpoints:
                ...
                if response.status_code == 200:
                    ...
                    print(f"  Fetched {len(data.get('data', []))} items from {endpoint}")
                else:
    ...
    def save_to_database(self, conn, all_data):
        ...
        for source_name, data_list in all_data.items():
            ...
            for item in data_list:
                try:
                    # Generate unique ID
                    unique_id = hashlib.md5(f"{source_name}_{item.get('id', 'unknown')}".encode()).hexdigest()
                    ...
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, [
                        unique_id, source_name, name, details, set_number, year,
                        theme, pieces, 0, price, rating
                    ])
